...
import { sentenceConllToJson, sentenceJsonToConll, emptyTreeJson, returnTokensInOrder, // TODO: try other enhanced branch instead
getNodeFromTreeJson, } from 'conllup/lib/conll';
...
export const defaultSentenceSVGOptions = () => ({
...
export class SentenceSVG extends EventDispatcher {
    constructor(svgWrapper, reactiveSentence, sentenceSVGOptions) {
        super();
        ...
        this.options = defaultSentenceSVGOptions();
    ...
    plugDiffTree(teacherReactiveSentence) {
        var _a, _b;
        if ((_a = teacherReactiveSentence === null || teacherReactiveSentence === void 0 ? void 0 : teacherReactiveSentence.state) === null || _a === void 0 ? void 0 : _a.treeJson) {
            this.teacherTreeJson = (_b = teacherReactiveSentence === null || teacherReactiveSentence === void 0 ? void 0 : teacherReactiveSentence.state) === null || _b === void 0 ? void 0 : _b.treeJson;
            this.drawTree();
    ...
    populateOrderOfTokens() {
        ...
        let stack = [];
        let orderOfTokens = [];
        const tokensInOrder = returnTokensInOrder(this.treeJson, this.options.drawEnhancedTokens, this.options.drawGroupTokens);
    ...
    populateLevels() {
        ...
        const levelsArray = new Array(headsIdsArray.length).fill(ROOT_ID_IN_SVG);
    ...
    getLevel(levelsArray, headsIdsArray, index, start, end) {
    ...
    computeCurrentMaxHeight() {
        ...
        for (const tokenSVG of this.tokenSVGs) {
            const tokenSVGHeight = Math.max(...Object.values(tokenSVG.snapElements).map((snapElement) => snapElement.getBBox().y2));
        ...
        if (this.options.packages !== null) {
            ...
            for (const tokenSVG of this.tokenSVGs) {
                if (modifiedNodesId.includes(tokenSVG.tokenJson.ID.toString())) {
                    const modifiedNode = this.options.packages.modified_nodes.filter((mNode) => mNode.id === tokenSVG.tokenJson.ID.toString())[0] ||
                        null;
        ...
        for (const tokenIndex in teacherTreeJson.nodesJson) {
            if (teacherTreeJson.nodesJson[tokenIndex]) {
                for (const tag in corrects) {
                    if (teacherTreeJson.nodesJson[tokenIndex][tag] !== '_' &&
...
class TokenSVG {
    ...
    createSnap(snapSentence, shownFeatures, startX, startY, lastToken, prevX, anchored, ordered, subsequent) {
        ...
        for (const feature in this.tokenJson) {
            ...
            if (this.tokenJson[feature]) {
                if (feature.split('.').length >= 2) {
                    ...
                    featureText = this.tokenJson[feature];
                }
            }
            else {
                featureText = '';
    ...
    addButton(icon, x, y, className, active = true) {
    ...
    drawEnhancedRelation(snapSentence, headCoordX, levelHeight, depsInfo, Y_start) {
        // const heightArc = this.startY - this.ylevel * levelHeight;
    ...
    showDiff(otherTokenJson) {
        if ((this.tokenJson.HEAD === 0 || this.tokenJson.HEAD >= 1) &&
    ...
    stopDrag(e) {
        ...
        if (this.sentenceSVG.hovered) {
            this.sentenceSVG.tokenSVGs[this.sentenceSVG.tokenIndexToSvgPosition[this.sentenceSVG.hovered]].snapElements['FORM'].removeClass('glossy');
            this.sentenceSVG.hovered = '';
        }
...
/////////////// SVG ELEMENT ////////////////
///////////////             ////////////////
function getArrowheadPath(X_depBoxCenter, Y_depBoxUpperBound) {
    ...
    const lefttop = '0,0' + -arrowheadsize / 2 + ',' + -arrowheadsize * 1.5 + ' ' + -arrowheadsize / 2 + ',' + -arrowheadsize * 1.5;
    const righttop = arrowheadsize / 2 +
        ',' +
        arrowheadsize / 2 +
        ' ' +
        arrowheadsize / 2 +
...
function getArcPath(X_start, X_end, Y_start, Y_end, Y_height) {
...
function getArcPathRoot(X_depBoxCenter, Y_depBoxUpperBound) {
